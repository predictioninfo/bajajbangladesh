...
class TimesheetController extends Controller
{
    ...
    public function attendanceIndex()
    {
        ...
        if (Package::where('id', '=', Auth::user()->com_pack)->whereRaw(
            ...
            if (Permission::where('permission_com_id', '=', Auth::user()->com_id)->where(
                'permission_role_id',
                '=',
                Auth::user()->role_id
            )->whereRaw(
                ...
                try {
                    ...
                    if ($ipCheck == null) {
                        ...
                        ])->get($ipCheck->ip . 'iclock/api/transactions/', [
                            'page_size' => 200,
                    ...
                    // Iterate through the response data and insert/update records
                    foreach ($responseData['data'] as $transactionData) {
                        $empCode = $transactionData['emp_code'];

                        // Check if the record already exists
                        $existingRecord = DeviceAttendance::where('emp_code', $empCode)
                            ->where('attendance_date', Carbon::parse($transactionData['punch_time'])->toDateString())
                            ->first();

                        if ($existingRecord) {
                            // Update the clock_out time if needed
                            $existingRecord->clock_out = Carbon::parse($transactionData['punch_time'])->format('H:i:s');
                            $existingRecord->save();
                        } else {
                            // Insert a new record
                            $record = [
                                ...
                                'attendance_date' => Carbon::parse($transactionData['punch_time'])->toDateString(),
                                'clock_in' => Carbon::parse($transactionData['punch_time'])->format('H:i:s'),
                                'clock_out' => Carbon::parse($transactionData['punch_time'])->format('H:i:s'),
                                'punch_time' => $transactionData['punch_time'],
                                'area_alias' => $transactionData['area_alias'],
                                'terminal_sn' => $transactionData['terminal_sn'],
                                'terminal_alias' => $transactionData['terminal_alias'],
                                'verify_type_display' => $transactionData['verify_type_display'],
                            ];
                            DeviceAttendance::create($record);
                        }
                    }
                    ...
                    $tokenError = response()->json(['error' => $e->getMessage()], 500);
                    if ($tokenError) {
                        return redirect('set-ip')->with('message', 'Firstly set IP Address');
                    }
                }
                ...
                // Fetch data from DeviceAttendance which are store from Device api
                $machinetransactionData = DeviceAttendance::where('com_id', Auth::user()->com_id)->get();
                ...
                foreach ($machinetransactionData as $transaction) {
                    $users = DB::table('users')
                        ...
                        ->get();
                    // Matching emp_code with company_assigned_id from User table
                    foreach ($users as $user) {
                        $matchingUser = new \stdClass();
                        $matchingUser->id = $user->id;
                        $matchingUser->company_assigned_id = $transaction->emp_code;
                        $matchingUser->attendance_date = $transaction->attendance_date;
                        $matchingUser->clock_in = $transaction->clock_in;
                        $matchingUser->clock_out = $transaction->clock_out;
                ...
                foreach ($matchingUsers as $matchingUser) {
                    $user = DB::table('users')
                        ->where('company_assigned_id', $transaction->emp_code)
                        ->first();
                    $current_day_name = date('D', strtotime($transaction->attendance_date));
                    ...
                    $clockIntime = $transaction->clock_in;

                    $clockInDate = Carbon::createFromFormat('Y-m-d', $transaction->attendance_date);
                    ...
                    if (Attendance::where('employee_id', $matchingUser->id)->where('attendance_date', date('y-m-d'))->exists()) {
                        // update the latest attendance record with the clock-out time
                        $empCodeCounts = DeviceAttendance::where('com_id', Auth::user()->com_id)->get();
                        foreach ($empCodeCounts as $empCodeCount) {
                            if (Attendance::where('employee_id', $matchingUser->id)->where('attendance_date', date('y-m-d'))->exists()) {
                                $attendance = Attendance::where('employee_id', $matchingUser->id)->where('attendance_date', date('y-m-d'))->latest()->first();
                                //Initially Clock In & Out are same then clock out store will be null/empty and not same value then clock out will store updated value
                                if ($matchingUser->clock_in == $matchingUser->clock_out) {
                                    $clock_out = '';
                                } else {
                                    $clock_out = $matchingUser->clock_out;
                                }
                                $attendance->clock_out = $clock_out;
                                $attendance->clock_in = $matchingUser->clock_in;
                                if (date_create($matchingUser->clock_out) <= date_create($shift_out)) {
                                    if ($shift_in != 0 && $shift_out != 0) {
                                        $attendance->early_leaving = date_create($matchingUser->clock_out)->diff(date_create($shift_out))->format('%H:%i:%s');
                                    }
                                }
                                ...
                                if (MonthlyAttendance::where('monthly_employee_id', $matchingUser->id)->whereMonth('attendance_month', '=', $current_month)->whereYear('attendance_year', '=', $current_year)->exists()) {
                                    ...
                                    $monthly_attendance->attendance_month = $matchingUser->attendance_date;
                                    $monthly_attendance->attendance_year = $matchingUser->attendance_date;
                                    if ($current_date_number == 1) {
                                        $monthly_attendance->day_one = "P";
                                    } elseif ($current_date_number == 2) {
                                        $monthly_attendance->day_two = "P";
                        ...
                        $attendance->attendance_date = $matchingUser->attendance_date;
                        $attendance->clock_in = $matchingUser->clock_in;
                        $attendance->clock_out = $clock_out;
                        $attendance->check_in_out = 1;
                        ...
                        if (date_create($clockIntime) >= date_create($shift_in)) {
                            if ($shift_in != 0 && $shift_out != 0) {
                                ...
                                if ($current_time_in_seconds >= $office_late_time_config_in_seconds) {
                                    ...
                                    if (LateTime::where('late_time_employee_id', $matchingUser->id)->where('late_time_date',  $matchingUser->attendance_date)->exists()) {

                                        $late_times = LateTime::where('late_time_employee_id', $matchingUser->id)->where('late_time_date',  $matchingUser->attendance_date)->get('id');
                                        ...
                                        foreach ($late_times as $late_times_value) {
                                            ...
                                            $late_time->late_time_date =  $matchingUser->attendance_date;
                                            $late_time->save();
                                        }
                                    } else {
    ...
    public function manageLeaveIndex()
    {
        ...
        // $leave_types = LeaveType::where('leave_type_company_id', Auth::user()->com_id)->get(['id', 'leave_type', 'allocated_day', 'activation_days']);
        ...
        if (Auth::user()->company_profile == "Yes" || Auth::user()->userrole->roles_admin_status == "Yes") {
            $leaves = Leave::join('users', 'leaves.leaves_employee_id', '=', 'users.id')
                ...
                // ->select('leaves.*', 'users.first_name', 'users.company_assigned_id', 'users.last_name', 'departments.department_name', 'designations.designation_name', 'leave_types.leave_type')
                // ->where('leaves_company_id', '=', Auth::user()->com_id)
                // ->orderBy("id", "desc")
                // ->get();
                //######### new code
